...
SELECT 
    EXTRACT(YEAR FROM 
        CASE 
            WHEN date_added LIKE '%,%' THEN TO_DATE(date_added, 'Month DD, YYYY')
            WHEN date_added LIKE '%-%-%' THEN TO_DATE(date_added, 'DD-Mon-YY')
        END
    ) AS year,
	COUNT(*) AS yearly_content,
    ROUND(COUNT(*)::numeric/(SELECT COUNT(*) FROM netflix WHERE country ILIKE 'India')::numeric *100,2) AS total_content
FROM netflix
